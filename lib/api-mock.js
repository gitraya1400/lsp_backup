// File: frontend-lms-v1/lib/api-mock.js
...
export async function mockGetHasilAkhir(asesiId) {
  ...
  const statusAkhir = statusAkumulasiTeori === "KOMPETEN" && hasilPraktikum === "KOMPETEN" && hasilUnjukDiri === "KOMPETEN" ? "KOMPETEN" : "BELUM KOMPETEN";
  ...
  if (skemaId === "DS") {
    ...
    const statusAkhirBaru = statusTeoriBaru === "KOMPETEN" && hasilPraktikum === "KOMPETEN" && hasilUnjukDiri === "KOMPETEN" ? "KOMPETEN" : "BELUM KOMPETEN";
...
export async function mockDeleteSoal(skemaId, soalId) {
  ...
  // 1) Cek di soal praktikum gabungan per skema
  const praktikum = allPraktikumSoalDb.get(skemaId);
  if (praktikum && praktikum.id === soalId) {
    allPraktikumSoalDb.delete(skemaId);
    return { success: true, removedFrom: "praktikum" };
  }
  if (praktikum && praktikum.id) {
    // if praktikum stored as object and id differs, fallthrough
  }
...
// Tambahkan database materi in-memory
const allMateriDb = new Map();

/* Inisialisasi materi contoh untuk setiap unit agar halaman pembelajaran menampilkan materi */
(function initMateriForUnits() {
  const defaultPdfName = (unitId) => `${unitId}-materi.pdf`;
  const defaultVideoUrl = "https://youtu.be/NqFsSUu-4pk?si=97PcDo73aejJvoph";
  const defaultLinkUrl = "https://www.stis.ac.id/";

  for (const [skemaId, units] of allUnitsDb.entries()) {
    units.forEach((unit, idx) => {
      const unitId = unit.id;
      if (allMateriDb.has(unitId)) return;

      const items = [
        {
          id: `${unitId}-m1`,
          unitId,
          judul: `Pengenalan: ${unit.judul}`,
          jenis: "VIDEO",
          urlKonten: defaultVideoUrl,
          fileMeta: null,
          urutan: 1,
        },
        {
          id: `${unitId}-m2`,
          unitId,
          judul: `Slide: ${unit.judul}`,
          jenis: "PDF",
          urlKonten: `/api/mock-download/${defaultPdfName(unitId)}`,
          fileMeta: { id: `f-${unitId}-m2`, nama: defaultPdfName(unitId), url: `/api/mock-download/${defaultPdfName(unitId)}`, size: "120 KB" },
          urutan: 2,
        },
        {
          id: `${unitId}-m3`,
          unitId,
          judul: `Referensi Eksternal: ${unit.judul}`,
          jenis: "LINK",
          urlKonten: defaultLinkUrl,
          fileMeta: null,
          urutan: 3,
        },
      ];

      allMateriDb.set(unitId, items);
    });
  }
})();

/**
 * [ADMIN] Simulasi: POST /api/admin/materi?unitId=...
 */
export async function mockCreateMateri(unitId, materiData) {
  await delay(300);
  if (!unitId) throw new Error("unitId wajib");
  const list = allMateriDb.get(unitId) || [];
  const id = `${unitId}-m${Date.now()}`;

  // Validasi: jika PDF wajib berekstensi .pdf
  if (materiData.jenis === "PDF" && materiData.file && !materiData.file.nama.toLowerCase().endsWith(".pdf")) {
  ...
  const newMateri = {
    id,
    unitId,
    judul: materiData.judul || `Materi ${list.length + 1}`,
    jenis: materiData.jenis || "VIDEO",
    urlKonten: materiData.urlKonten || (materiData.file?.url ?? ""),
    fileMeta: materiData.file || null,
    urutan: list.length + 1,
    createdAt: new Date(),
  };

  list.push(newMateri);
  allMateriDb.set(unitId, list);

  // update materiCount pada unit
  const units = Array.from(allUnitsDb.values()).flat();
  const unit = units.find((u) => u.id === unitId);
  if (unit) unit.materiCount = (allMateriDb.get(unitId) || []).length;

  return newMateri;
}

/**
 * [ADMIN] Simulasi: PUT /api/admin/materi/:id
 */
export async function mockUpdateMateri(unitId, materiId, updates) {
  ...
  // Jika mengganti file, validasi ekstensi untuk PDF
  if (updates.jenis === "PDF" && updates.file && !updates.file.nama.toLowerCase().endsWith(".pdf")) {
    throw new Error("File harus berformat PDF");
  }
  ...
  const merged = {
    ...list[idx],
    ...updates,
    fileMeta: updates.file !== undefined ? updates.file : list[idx].fileMeta,
    updatedAt: new Date(),
  };
  list[idx] = merged;
  allMateriDb.set(unitId, list);

  // update materiCount pada unit (tidak berubah biasanya)
  const units = Array.from(allUnitsDb.values()).flat();
  const unit = units.find((u) => u.id === unitId);
  if (unit) unit.materiCount = (allMateriDb.get(unitId) || []).length;
...
/**
 * [ADMIN] Simulasi: DELETE /api/admin/materi/:id
 */
