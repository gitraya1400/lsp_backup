...
export async function mockCreateMateri(unitId, materiData) {
  ...
  const newMateri = {
    ...
    urlKonten: materiData.urlKonten,
  ...
  // Update materiCount di unit
  const units = Array.from(allUnitsDb.values()).flat();
  const unit = units.find(u => u.id === unitId);
  if (unit) unit.materiCount = list.length;
...
export async function mockUpdateMateri(unitId, materiId, updates) {
  ...
  // Validasi jenis file saat update
  ...
  const updated = { 
    ...list[idx], 
    ...updates,
    fileMeta: updates.file || list[idx].fileMeta // Pertahankan file lama jika tidak diupdate
  };
